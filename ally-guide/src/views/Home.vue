<template>
  <div class="Home">
    <div class="border-bottom border-white">
      <div class="container text-center py-5 mb-4">
        <img src="logo.png" class="w-100" style="max-width:180px;" />
        <h1 class="h1 text-black" style="vertical-align: bottom;">
          A Guide to being an Ally:
        </h1>
        <p>
          Ally.guide is a web app hoping to support and strengthen racial
          justice movements by guiding folx to become better allies by:
        </p>
        <br />
        <h2 class="h4 text-gray-dark font-weight-light">
          Its time. This is how.
        </h2>
      </div>
    </div>
    <div style="background-color:#892819" class="border-bottom border-white">
      <div class="container text-center">
        <div class="box box1">Educate <a class="toggle"></a></div>
        <div class="box box2"><a href="/donate"> Donate </a></div>
        <div class="box box3"><a href="/elevate">Elevate </a></div>
        <div class="box box4"><a href="/collaborate">Collaborate </a></div>
      </div>
      <div class="banner bg-black">Stuff shown on hover</div>
    </div>
  </div>
</template>

<script>
export default {
  name: "Home",
  components: {}
};
</script>
